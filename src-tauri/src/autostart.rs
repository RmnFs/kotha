...
mod macos {
    ...
    mod tests {
        ...
        fn launch_agent_path_matches_auto_launch_crate() {
            let path = plugin_launch_agent_path(Path::new("/Users/someone"), "Kotha");
            assert_eq!(
                path,
                Path::new("/Users/someone/Library/LaunchAgents/Kotha.plist")
            );
        }
        ...
        fn removes_existing_launch_agent() {
            ...
            let plist = dir.path().join("Kotha.plist");
        ...
        fn missing_launch_agent_is_a_no_op() {
            let dir = tempfile::tempdir().unwrap();
            remove_launch_agent_file(&dir.path().join("Kotha.plist"));
        }
    }
}
