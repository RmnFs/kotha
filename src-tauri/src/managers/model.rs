...
/// The base code Kotha matches a language *intent* on: a tag's primary subtag,
...
/// Bridges hf-hub's async download progress to Kotha's `model-download-progress`
...
impl ModelManager {
    ...
    /// models dropped in or downloaded outside Kotha show up without a restart.
    ...
    fn discover_custom_transcribe_models(
        ...
        for entry in fs::read_dir(models_dir)? {
            ...
            // Kotha-downloaded one. Legacy `.bin` files have no GGUF header, so
    ...
    /// shared Hugging Face cache, so models downloaded by Kotha (or any other
