...
#
# On Linux/Windows, Kotha builds transcribe-cpp in its `dynamic-backends`
...
# them. This script copies them, as a co-located set, next to where the `kotha`
# binary will find them at runtime:
#   - the loader resolves `libtranscribe` via the $ORIGIN-relative rpath baked
#     into `kotha` (see src-tauri/build.rs);
